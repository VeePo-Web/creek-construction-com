...
const CedarCTA = ({ to, preselectServices, children, variant = "primary", className, onActivate }: CedarCTAProps) => {
  ...
  const secondaryClass = cn(
    "text-minimal text-cedar hover:text-cedar-hover transition-[color,background-color,padding] duration-500 group/link inline-flex items-center gap-2 min-h-[44px] py-2 px-1 rounded-sm",
  ...
  const inner = (
    <>
      <span>{children}</span>
      {variant === "secondary" ? (
        <span className="inline-block w-4 h-px bg-gradient-to-r from-cedar to-cedar/60 group-hover/link:w-10 transition-[width] duration-500" />
      ) : (
